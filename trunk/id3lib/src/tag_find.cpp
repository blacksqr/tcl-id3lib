...
ID3_Frame *ID3_TagImpl::FindTXXXFrame(dami::String description) const
{
	ID3_Frame *frame = NULL;
	ID3_Frame *temp = NULL;
	// reset the cursor if it isn't set
	...
	for (int iCount = 0; iCount < 2 && frame == NULL; iCount++)
	{
		...
		for (const_iterator cur = begin; cur != end; ++cur)
		{
			if ((*cur != NULL) && ((*cur)->GetID() == ID3FID_USERTEXT))
			{
				// We've found a valid frame.  Set the cursor to be the next element
				temp = *cur;
				if (description.compare(temp->Field(ID3FN_DESCRIPTION).GetRawText())==NULL)
				{
					// Now lets see if we can find the Description of TXXX frame in current TXXX frame
					frame = *cur;
					_cursor = ++cur;
					break;
				}
